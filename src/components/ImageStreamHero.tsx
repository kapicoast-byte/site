 ...
 *
 * Ported from the 21st.dev component: the geometry below is unchanged,
 * but this project has no Tailwind and no `cn` helper, so the handful
 * of utility classes became plain style objects and two rules in
 * globals.css. Nothing about the maths depends on them.
 * ─────────────────────────────────────────────────────────────── */
...
export function ImageStreamHero({
  ...
  const css = React.useMemo(
    () =>
      ...
      //
      // Duration and iteration-count have to be restated with !important
      // because globals.css caps every animation at 1ms and one iteration
      // under reduced motion. That is right for a decorative loop but wrong
      // here: at 1ms every card has already finished, so they all land on the
      // 100% keyframe and stack into a single pile at the exit. Restoring the
      // timing lets each negative delay place its card along the corridor, and
      // play-state then holds the whole thing still.
              ...
              return (
                ...
                >
                  {img ? (
                    ...
                    /* Eager, not lazy. Every card sits inside a 3D transform
                       and most are scaled to a few pixels at the vanishing
                       point, so the lazy heuristic decides they are not worth
                       fetching and the corridor runs with empty cards. There
                       are only `cards` distinct images and they are small
                       WebP, so loading them up front is the cheaper mistake. */
                    <img
                      src={img.src}
                      alt={img.alt ?? ""}
                      loading="eager"
